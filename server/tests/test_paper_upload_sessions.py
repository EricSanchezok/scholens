...
class MemoryUploadGateway:
    ...
    def consume(self, **values: object) -> None:
        del values
        assert self.record is not None
        self.record = replace(self.record, status="consumed", lease_expires_at=None)
...
def test_upload_lifecycle_delegates_claim_consume_and_retryable_release() -> None:
    ...
    sessions.release(actor=_actor(), upload_id=prepared.upload_id, failed=False)
    assert gateway.record is not None
    assert gateway.record.status == "prepared"

    sessions.claim(actor=_actor(), upload_id=prepared.upload_id)
    sessions.consume(actor=_actor(), upload_id=prepared.upload_id)
